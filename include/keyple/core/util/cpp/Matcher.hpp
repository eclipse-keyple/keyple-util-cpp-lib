...
public:
     ...
     * of the state machine is called. The state machine will hold the state of
     * the match as it proceeds in this matcher.
     */
     ...
     * sequence looking for matches of the pattern.  Characters that are not part
     * of any match are appended directly to the result string; each match is
     ...
     *
     * If the match succeeds then more information can be obtained viw the start,
     * end, group methods.
     *
     * @return true if, and only if, a subsequence of the input sequences matches
     * this matcher's patern.
     */
    bool find();
     ...
     *
     * If the match succeeds then more information can be obtained viw the start,
     * end, group methods, and subsequent invocations of the find() method will
     * start at the first character not matched by this match.
     *
     * @return true if, and only if, a subsequence of the input sequences matches
     * this matcher's patern.
     ...
     * machine is called. The state machine will hold the state of the match as it
     * proceeds in this matcher.
     *
     * Matcher.from is not set here, because it is the 'hard' boundary of the
     * start of the search which anchors will set to. The from param is the 'soft'
     * boundary of the start of the search, meaning that the regex tries to match
     * at that index but won't match there. Subsequent calls to the search methods
     ...
     * Resets this matcher.
     *
     * <p> Resetting a matcher discards all of its explicit states information and
...
private:
     ...
     * have to consume all of the input. ENDANCHOR is the mode used for matching
     * all the input.
     */
    int ENDANCHOR = 1;
     ...
     * where groups begin. The nodes themselves are stateless, so they rely on
     * this field to hold state during a match.
     */
