...
public:
    ...
    /**
     * By default Pthreads are joinable. meaning you can wait for them to complete
     * with a call to pthread_join(). The Thread class join method checks to see
     * if the thread is running, then calls this function to wait for the thread
     * to complete. If the call is successful the thread is marked as detached
...
private:
     ...
     * data structure which will be passed to the runThread() function when it is
     * called. Since the input argument to the runThread() is the Thread class
     * this pointer, we can cast it to a Thread pointer then use it to call the
     * Thread::run() method. Due to polymorphism, the Thread subclass run() method
