 ...
 **************************************************************************************************/
#ifndef KEYPLE_CORE_UTIL_ANY_HPP
#define KEYPLE_CORE_UTIL_ANY_HPP
#pragma once
#include <algorithm>
#include <stdexcept>
#include <type_traits>
...
public:
    ...
    /// program is ill-formed. This is because an `any` may be copy constructed
    /// into another `any` at any time, so a copy should always be allowed.
    template <
        typename ValueType,
    ...
    /// Has the same effect as any(rhs).swap(*this). No effects if an exception
    /// is thrown.
    any&
...
private:  // Storage and Virtual Method Table
    ...
    struct vtable_type {
        ...
        /// unitialized dest union. As such, both inner objects will have the
        /// same state, but on separate memory locations.
        void (*copy)(const storage_union& src, storage_union& dest);

        /// Moves the storage from src to the yet unitialized dest union.
        /// The state of src after this call is unspecified, caller must ensure
        /// not to use src anymore.
    ...
    struct vtable_stack {
        ...
        move(storage_union& src, storage_union& dest) noexcept
        {
            ...
            // constructor, so this move constructor will never throw a
            // exception.
...
protected:
    ...
    /// of the type infos, otherwise does an actual comparision. Checking
    /// addresses is only a valid approach when there's no interaction with
    /// outside sources (other shared libraries and such).
    static bool
...
inline void
swap(
    keyple::core::util::cpp::any& lhs,
    keyple::core::util::cpp::any& rhs) noexcept
{
    lhs.swap(rhs);
}
