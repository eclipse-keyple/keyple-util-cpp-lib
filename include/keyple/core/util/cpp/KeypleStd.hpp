...
// when using a compiler that does not support C++17, define our own
// implementation of reinterpret_pointer_cast
...
{
    os << static_cast<int>(v) << "(0x" << std::uppercase << std::hex
       << std::setfill('0') << std::setw(2) << static_cast<int>(v) << ")";
...
operator<<(std::ostream& os, const std::vector<uint8_t>& v)
{
    for (const auto val : v) {
        os << static_cast<int>(val) << std::uppercase << std::hex
           << std::setfill('0') << std::setw(2) << static_cast<int>(val);
    }
...
{
    for (const auto val : v) {
        os << val << std::uppercase << std::hex << std::setfill('0')
           << std::setw(8) << static_cast<int>(val);
    }
