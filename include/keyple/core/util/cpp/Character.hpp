...
public:
     ...
     * version. Note that non-breaking spaces are not considered whitespace.
     * Note also that line separators are considered whitespace; see
     * isSpaceChar() for an alternative.
     */
    static bool
    isWhitespace(int codePoint)
    {
        ...
        /* Exclude General Punctuation's non-breaking spaces (which includes
         * FIGURE SPACE). */
        ...
        if (codePoint <= 0xffff) {
            ...
            return codePoint <= 0x200a || codePoint == 0x2028
                   ...
                   || codePoint
                          == 0x3000; /* ...or CJK Symbols and Punctuation? */
        }
