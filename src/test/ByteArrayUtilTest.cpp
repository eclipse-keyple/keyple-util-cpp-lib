...
const char* HEXSTRING_ODD = "0102030";
const char* HEXSTRING_BAD = "010203ABGH8+";
const char* HEXSTRING_GOOD = "1234567890ABCDEFFEDCBA0987654321";
...
TEST(
    ByteArrayUtilTest,
    extractBytes_byteArray_whenBitOffsetIsOutOfRange_shouldThrowAIOOBE)
{
    EXPECT_THROW(
        ByteArrayUtil::extractBytes(std::vector<uint8_t>(1), 16, 1),
        ArrayIndexOutOfBoundsException);
}

TEST(
    ByteArrayUtilTest,
    extractBytes_byteArray_whenBitOffsetIsOutOfRange_shouldThrowAIOOBE2)
{
    EXPECT_THROW(
        ByteArrayUtil::extractBytes(std::vector<uint8_t>(1), 9, 1),
        ArrayIndexOutOfBoundsException);
}

TEST(
    ByteArrayUtilTest,
    extractBytes_byteArray_whenBitOffsetIsNegative_shouldThrowAIOOBE)
{
    EXPECT_THROW(
        ByteArrayUtil::extractBytes(std::vector<uint8_t>(1), -8, 1),
        ArrayIndexOutOfBoundsException);
}

TEST(
    ByteArrayUtilTest,
    extractBytes_byteArray_whenBitOffsetIsNegative_shouldThrowAIOOBE2)
{
    EXPECT_THROW(
        ByteArrayUtil::extractBytes(std::vector<uint8_t>(1), -1, 1),
        ArrayIndexOutOfBoundsException);
}

TEST(
    ByteArrayUtilTest,
    extractBytes_byteArray_whenNbBytesIsOutOfRange_shouldThrowAIOOBE)
{
    EXPECT_THROW(
        ByteArrayUtil::extractBytes(std::vector<uint8_t>(1), 0, 2),
        ArrayIndexOutOfBoundsException);
}

TEST(
    ByteArrayUtilTest,
    extractBytes_byteArray_whenNbBytesIsOutOfRange_shouldThrowAIOOBE2)
{
    EXPECT_THROW(
        ByteArrayUtil::extractBytes(std::vector<uint8_t>(1), 1, 2),
        ArrayIndexOutOfBoundsException);
}

TEST(
    ByteArrayUtilTest,
    extractBytes_byteArray_whenNbBytesIsNegative_shouldThrowAIOOBE)
{
    EXPECT_THROW(
        ByteArrayUtil::extractBytes(std::vector<uint8_t>(1), 0, -1),
        NegativeArraySizeException);
}

TEST(
    ByteArrayUtilTest,
    extractBytes_byteArray_whenNbBytesIsNegative_shouldThrowAIOOBE2)
{
    EXPECT_THROW(
        ByteArrayUtil::extractBytes(std::vector<uint8_t>(1), 1, -1),
        NegativeArraySizeException);
...
TEST(
    ByteArrayUtilTest,
    extractBytes_byteArray_whenBitOffsetIsMultipleOf8_shouldBeSuccessful)
{
    const std::vector<uint8_t> src = {0xF1, 0xF2, 0xF3};

    ASSERT_TRUE(Arrays::equals(ByteArrayUtil::extractBytes(src, 8, 1), {0xF2}));
    ASSERT_TRUE(
        Arrays::equals(ByteArrayUtil::extractBytes(src, 8, 2), {0xF2, 0xF3}));
}

TEST(
    ByteArrayUtilTest,
    extractBytes_byteArray_whenBitOffsetIsNotMultipleOf8_shouldBeSuccessful)
{
    const std::vector<uint8_t> src = {0xF1, 0xF2, 0xF3};

    ASSERT_TRUE(Arrays::equals(ByteArrayUtil::extractBytes(src, 6, 1), {0x7C}));
    ASSERT_TRUE(
        Arrays::equals(ByteArrayUtil::extractBytes(src, 6, 2), {0x7C, 0xBC}));
    ASSERT_TRUE(Arrays::equals(ByteArrayUtil::extractBytes(src, 3, 1), {0x8F}));
}

TEST(
    ByteArrayUtilTest, extractBytes_number_AndNbBytesIsNegative_shouldThrowNASE)
{
    EXPECT_THROW(
...
TEST(
    ByteArrayUtilTest,
    extractBytes_number_AndNbBytesIs0_shouldReturnAnEmptyArray)
{
...
TEST(
    ByteArrayUtilTest,
    extractBytes_number_AndNbBytesIs1to8_shouldExtractLastBytes)
{
    /* Short */
    const uint16_t shortNumber = 0xFF22;
    ASSERT_TRUE(
        Arrays::equals(ByteArrayUtil::extractBytes(shortNumber, 1), {0x22}));
    ASSERT_TRUE(Arrays::equals(
        ByteArrayUtil::extractBytes(shortNumber, 2), {0xFF, 0x22}));

    /* Integer */
    const int intNumber = 0xFF223344;
    ASSERT_TRUE(Arrays::equals(
        ByteArrayUtil::extractBytes(static_cast<uint64_t>(intNumber), 1),
        {0x44}));
    ASSERT_TRUE(Arrays::equals(
        ByteArrayUtil::extractBytes(static_cast<uint64_t>(intNumber), 2),
        {0x33, 0x44}));
    ASSERT_TRUE(Arrays::equals(
        ByteArrayUtil::extractBytes(static_cast<uint64_t>(intNumber), 3),
        {0x22, 0x33, 0x44}));
    ASSERT_TRUE(Arrays::equals(
        ByteArrayUtil::extractBytes(static_cast<uint64_t>(intNumber), 4),
        {0xFF, 0x22, 0x33, 0x44}));

    /* Long */
    const uint64_t longNumber = 0xFF22334455667788L;
    ASSERT_TRUE(
        Arrays::equals(ByteArrayUtil::extractBytes(longNumber, 1), {0x88}));
    ASSERT_TRUE(Arrays::equals(
        ByteArrayUtil::extractBytes(longNumber, 2), {0x77, 0x88}));
    ASSERT_TRUE(Arrays::equals(
        ByteArrayUtil::extractBytes(longNumber, 3), {0x66, 0x77, 0x88}));
    ASSERT_TRUE(Arrays::equals(
        ByteArrayUtil::extractBytes(longNumber, 4), {0x55, 0x66, 0x77, 0x88}));
    ASSERT_TRUE(Arrays::equals(
        ByteArrayUtil::extractBytes(longNumber, 5),
        {0x44, 0x55, 0x66, 0x77, 0x88}));
    ASSERT_TRUE(Arrays::equals(
        ByteArrayUtil::extractBytes(longNumber, 6),
        {0x33, 0x44, 0x55, 0x66, 0x77, 0x88}));
    ASSERT_TRUE(Arrays::equals(
        ByteArrayUtil::extractBytes(longNumber, 7),
        {0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88}));
    ASSERT_TRUE(Arrays::equals(
        ByteArrayUtil::extractBytes(longNumber, 8),
...
{
    EXPECT_THROW(
        ByteArrayUtil::extractShort(std::vector<uint8_t>(2), -1),
        ArrayIndexOutOfBoundsException);
}

TEST(
    ByteArrayUtilTest,
    extractShort_whenOffsetIsGreaterThanSrcLengthMinus2_shouldThrowAIOOBE)
{
    EXPECT_THROW(
        ByteArrayUtil::extractShort(std::vector<uint8_t>(2), 1),
        ArrayIndexOutOfBoundsException);
...
{
    EXPECT_THROW(
        ByteArrayUtil::extractInt(std::vector<uint8_t>(1), -1, 1, true),
        ArrayIndexOutOfBoundsException);
}

TEST(
    ByteArrayUtilTest,
    extractInt_whenOffsetIsGreaterThanSrcLengthMinusNbBytes_shouldThrowAIOOBE)
{
    EXPECT_THROW(
        ByteArrayUtil::extractInt(std::vector<uint8_t>(1), 1, 1, true),
        ArrayIndexOutOfBoundsException);
...
{
    EXPECT_THROW(
        ByteArrayUtil::extractInt(std::vector<uint8_t>(1), 0, 2, true),
        ArrayIndexOutOfBoundsException);
}
...
{
    EXPECT_THROW(
        ByteArrayUtil::extractLong(std::vector<uint8_t>(1), -1, 1, true),
        ArrayIndexOutOfBoundsException);
}

TEST(
    ByteArrayUtilTest,
    extractLong_whenOffsetIsGreaterThanSrcLengthMinusNbBytes_shouldThrowAIOOBE)
{
    EXPECT_THROW(
        ByteArrayUtil::extractLong(std::vector<uint8_t>(1), 1, 1, true),
        ArrayIndexOutOfBoundsException);
...
{
    EXPECT_THROW(
        ByteArrayUtil::extractLong(std::vector<uint8_t>(1), 0, 2, true),
        ArrayIndexOutOfBoundsException);
}
...
TEST(ByteArrayUtilTest, extractLong_whenInputIsOk_shouldBeSuccessful)
{
    ...
    ASSERT_EQ(
        ByteArrayUtil::extractLong(src, 1, 8, false), 0xF2F3F4F5F6F7F8F9L);
}
...
TEST(ByteArrayUtilTest, copyBytes_whenOffsetIsNegative_shouldThrow)
{
    ...
    EXPECT_THROW(
        ByteArrayUtil::copyBytes(0, dest, -1, 0),
        ArrayIndexOutOfBoundsException);
}
...
TEST(ByteArrayUtilTest, copyBytes_whenOffsetIsOutOfRange_shouldThrowAIOOBE)
{
    ...
    EXPECT_THROW(
        ByteArrayUtil::copyBytes(0, dest, 1, 1),
        ArrayIndexOutOfBoundsException);
}
...
TEST(ByteArrayUtilTest, copyBytes_whenNbBytesIsOutOfRange_shouldThrow)
{
    ...
    EXPECT_THROW(
        ByteArrayUtil::copyBytes(0, dest, 0, 2),
        ArrayIndexOutOfBoundsException);
}
...
TEST(ByteArrayUtilTest, copyBytes_whenOffsetAndNbBytesIsOutOfRange_shouldThrow)
{
    ...
    EXPECT_THROW(
        ByteArrayUtil::copyBytes(0, dest, 1, 2),
        ArrayIndexOutOfBoundsException);
}
...
TEST(ByteArrayUtilTest, copyBytes_whenSrcIsLong_shouldBeSuccess)
{
    ...
    ASSERT_EQ(
        dest,
        std::vector<uint8_t>({0x88, 0xF2, 0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8}));

    ByteArrayUtil::copyBytes(src, dest, 0, 2);

    ASSERT_EQ(
        dest,
        std::vector<uint8_t>({0x77, 0x88, 0xF3, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8}));

    ByteArrayUtil::copyBytes(src, dest, 0, 3);

    ASSERT_EQ(
        dest,
        std::vector<uint8_t>({0x66, 0x77, 0x88, 0xF4, 0xF5, 0xF6, 0xF7, 0xF8}));

    ByteArrayUtil::copyBytes(src, dest, 0, 4);

    ASSERT_EQ(
        dest,
        std::vector<uint8_t>({0x55, 0x66, 0x77, 0x88, 0xF5, 0xF6, 0xF7, 0xF8}));

    ByteArrayUtil::copyBytes(src, dest, 0, 5);

    ASSERT_EQ(
        dest,
        std::vector<uint8_t>({0x44, 0x55, 0x66, 0x77, 0x88, 0xF6, 0xF7, 0xF8}));

    ByteArrayUtil::copyBytes(src, dest, 0, 6);

    ASSERT_EQ(
        dest,
        std::vector<uint8_t>({0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0xF7, 0xF8}));

    ByteArrayUtil::copyBytes(src, dest, 0, 7);

    ASSERT_EQ(
        dest,
        std::vector<uint8_t>({0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0xF8}));
...
{
    EXPECT_THROW(
        ByteArrayUtil::fromHex(HEXSTRING_ODD), IllegalArgumentException);
}
...
{
    EXPECT_THROW(
        ByteArrayUtil::twoBytesToInt(BYTEARRAY_LEN_16, -1),
        ArrayIndexOutOfBoundsException);
}
...
{
    EXPECT_THROW(
        ByteArrayUtil::twoBytesToInt(BYTEARRAY_LEN_1, 0),
        ArrayIndexOutOfBoundsException);
}
...
{
    EXPECT_THROW(
        ByteArrayUtil::twoBytesToInt(BYTEARRAY_LEN_3, 2),
        ArrayIndexOutOfBoundsException);
}
...
{
    EXPECT_THROW(
        ByteArrayUtil::threeBytesToInt(BYTEARRAY_LEN_16, -1),
        ArrayIndexOutOfBoundsException);
}
...
{
    EXPECT_THROW(
        ByteArrayUtil::threeBytesToInt(BYTEARRAY_LEN_2, 0),
        ArrayIndexOutOfBoundsException);
}
...
{
    EXPECT_THROW(
        ByteArrayUtil::threeBytesToInt(BYTEARRAY_LEN_3, 1),
        ArrayIndexOutOfBoundsException);
}
...
{
    EXPECT_THROW(
        ByteArrayUtil::fourBytesToInt(BYTEARRAY_LEN_16, -1),
        ArrayIndexOutOfBoundsException);
}
...
{
    EXPECT_THROW(
        ByteArrayUtil::fourBytesToInt(BYTEARRAY_LEN_3, 0),
        ArrayIndexOutOfBoundsException);
}
...
{
    EXPECT_THROW(
        ByteArrayUtil::fourBytesToInt(BYTEARRAY_LEN_4, 1),
        ArrayIndexOutOfBoundsException);
}
