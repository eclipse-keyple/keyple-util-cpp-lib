...
TEST(ByteArrayUtilTest, extractBytes_number_AndNbBytesIs1to8_shouldExtractLastBytes) {
    ...
    ASSERT_TRUE(
        Arrays::equals(ByteArrayUtil::extractBytes(intNumber, 4), {0xFF, 0x22, 0x33, 0x44}));
...
TEST(
    ByteArrayUtilTest, extractLong_whenOffsetIsGreaterThanSrcLengthMinusNbBytes_shouldThrowAIOOBE) {
    EXPECT_THROW(
        ByteArrayUtil::extractLong(std::vector<uint8_t>(1), 1, 1, true),
        ArrayIndexOutOfBoundsException);
}
